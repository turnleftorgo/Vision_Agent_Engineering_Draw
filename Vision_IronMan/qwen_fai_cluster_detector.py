...
你不是在检测“FAI 圆圈附近的一块区域”，而是在恢复一条完整工程语义链：

FAI/SPC 标记 → 对应标注内容 → 完整引线或尺寸链 → 全部终端箭头/接触点
→ 被定义、测量或说明的完整局部物理特征。

最终 bbox 是上述整条链的最小完整轴对齐外接矩形。它既不能只框文字和 FAI，
也不能因为目标属于一个大型外壳就无条件框住整个外壳。

“完整局部物理特征”的边界必须按以下规则确定：

1. 单个孔、沉孔、槽、长圆孔、螺钉、凸台、缺口、台阶或局部开口：
   包含该特征的完整闭合轮廓、必要内部轮廓和与箭头接触的边界；不能只框箭头尖端
   附近的一小段线。
2. 一段边、壁、法兰、筋、安装耳或表面：沿连续轮廓追踪到清晰转角、端点、结构
   分界或另一自然边界。不要在连续轮廓中间截断，也不要扩张到与该标注无关的整个
   母体零件。
3. 线性、角度或距离尺寸：箭头可能落在尺寸线、延长线或见证线上。必须沿对应延长
   线追踪到实际被测的两个表面/边界，并包含两端被测几何及其测量跨度；尺寸文字和
   箭头本身不是物理目标。
4. 直径、半径或圆弧尺寸：包含所定义的完整孔、圆、圆弧、圆柱或槽形特征；只看到
   一个短弧或一侧边界不算完整。
5. 带数量或范围词的重复特征，例如 NX、ALL POINTS、HOLES、SLOTS 或一组连续孔位：
   如果标注明确同时适用于一组实例，必须包含该标注覆盖的全部可见实例和它们共同的
   局部承载区域，不能只选择其中一个孔或一个点。
6. 注释型 callout 或 note：从文字末端沿每一条相关引线分别追踪；同一标注有多个
   箭头时，所有箭头及所有对应目标都必须进入同一个 FAI 集群框。
7. 两个不同 FAI 指向同一物理区域时，仍输出两个独立对象；它们的 bbox 可以大量
   重叠。不要为了消除重叠而合并 FAI，也不要把共享目标从其中一个框中删除。

决定 bbox 前必须执行四边审计：检查拟议框的左、右、上、下边界。如果任何属于该
FAI 的文字、框格、引线、箭头、被测表面、重复实例或目标连续轮廓穿过边界，就继续
扩张该边；只有在四边都落在自然空白或明确结构终点之外时，才可认为框选完整。
...
8. 判断目标属于单一闭合特征、连续局部结构、尺寸两端，还是带 NX/ALL POINTS 的
   重复特征组，并据此确定正确的局部自然边界。
9. 对 FAI/SPC、标注、完整引出路径、箭头和完整局部目标特征求能够覆盖全部内容的
   轴对齐外接矩形，完整性优先于框的紧密程度。
10. 执行左、右、上、下四边审计；任何关联元素或目标轮廓被边界切断都必须扩大。

禁止使用以下捷径：

- 只框 FAI/SPC 和相邻文字，然后声称 complete=true；
- 把 dimension line、extension line、leader 或 arrowhead 当成物理目标；
- 看到箭头碰到大型外壳就直接框整个外壳，而不判断实际被定义的局部特征；
- 看到 NX、ALL POINTS 或一排重复孔槽时只框其中一个实例；
- 因为两个 FAI 的目标区域重叠而把它们合并；
- 因为辅助彩色标记缺失，就忽略原始黑色图纸中清晰可见的 FAI。
...
7. NX、ALL POINTS、HOLES、SLOTS 等重复特征是否包含了标注所覆盖的全部实例。
8. 线性或角度尺寸是否已经从尺寸箭头追踪到真正的两端被测表面。
9. bbox 是否停在目标的自然局部边界，而不是只截取目标种子，也不是无理由吞入整个母体零件。
10. 是否检查了没有任何彩色辅助标记、但在原始黑色图纸中明确可见的 FAI。"""
